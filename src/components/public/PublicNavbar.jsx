    ...
    <nav className={`fixed top-0 w-full z-50 transition-all duration-300 ${scrolled || !isLanding ? 'bg-white/90 backdrop-blur-md border-b border-[#E2E8F0] shadow-sm py-3' : 'bg-transparent py-5'}`}>
      <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
        <Link to="/" className="flex items-center gap-3 group">
          <span className="flex h-8 w-8 items-center justify-center rounded-[8px] bg-[#2563EB] text-white font-bold text-lg shadow-sm transition-transform group-hover:scale-105">D</span>
          <span className="font-bold text-slate-900 text-xl tracking-tight hidden sm:block">DISHA</span>
        </Link>
        
        <div className="hidden lg:flex items-center gap-8">
          <a href="/#home" className="text-sm font-semibold text-slate-600 hover:text-[#2563EB] transition-colors">Home</a>
          <a href="/#about" className="text-sm font-semibold text-slate-600 hover:text-[#2563EB] transition-colors">About</a>
          <Link to="/blogs" className={`text-sm font-semibold transition-colors ${location.pathname === '/blogs' ? 'text-[#2563EB]' : 'text-slate-600 hover:text-[#2563EB]'}`}>Blogs</Link>
          <Link to="/contact" className={`text-sm font-semibold transition-colors ${location.pathname === '/contact' ? 'text-[#2563EB]' : 'text-slate-600 hover:text-[#2563EB]'}`}>Contact</Link>
        </div>
