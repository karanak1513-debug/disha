    ...
    >
      ...
      <div className="flex-1 p-6 flex flex-col">
        ...
        <div className="mt-auto">
          {isApplied ? (
            <div className={`flex items-center justify-center gap-2 w-full py-3.5 rounded-xl text-xs font-bold ${STATUS_STYLE[app?.status]}`}>
              <CheckCircle2 className="h-4 w-4" /> {STATUS_ICON[app?.status]}
            </div>
          ) : isFull ? (
            ...
            <motion.button
              ...
              disabled={isApplying}
              className="w-full flex items-center justify-center gap-2 py-3.5 rounded-xl bg-[#2563EB] hover:bg-[#1D4ED8] text-white text-sm font-bold transition-all disabled:opacity-50 hover:shadow-lg hover:shadow-blue-500/25"
            >
              {isApplying ? (
                <>
                  <motion.span animate={{ rotate: 360 }} transition={{ duration: 1, repeat: Infinity, ease: "linear" }} className="h-4 w-4 border-2 border-white/30 border-t-white rounded-full" />
                  Processing...
                </>
              ) : (
